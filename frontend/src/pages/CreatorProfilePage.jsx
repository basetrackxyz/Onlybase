...
export default function CreatorProfilePage() {
  ...
  const gridPosts = posts.filter((p) => p.mediaUrl); // for media tab
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] flex items-stretch justify-center">
      <div className="relative w-full max-w-md min-h-screen bg-white flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="px-5 -mt-10 relative">
          <div className="flex items-end justify-between">
            <img
              src={creator.avatar}
              alt={creator.name}
              className="w-20 h-20 rounded-full ring-4 ring-white object-cover shadow-lg"
            />
          ...
          <div className="mt-3">
            <div className="flex items-center gap-1.5">
              <h1 className="font-display text-xl font-bold tracking-tight">{creator.name}</h1>
              {creator.verified && <VerifiedBadge size={18} />}
            </div>
            <p className="text-[13px] text-gray-500 mb-2">@{creator.handle}</p>
            <p className="text-[14px] text-black leading-relaxed mb-3">{creator.bio}</p>

            <div className="flex items-center gap-4 text-[12px] text-gray-500 mb-4">
              <span>
                <strong className="text-black">{(creator.followers / 1000).toFixed(1)}k</strong>{" "}
                followers
              </span>
              <span>
                <strong className="text-black">{creator.posts}</strong> posts
              </span>
        ...
        {/* Tabs */}
        <div className="flex border-b border-gray-100 px-5 sticky top-0 bg-white z-10">
          ...
          {tab === "posts" && (
            <div>
              {creatorPosts.length === 0 ? (
                <div className="text-center text-gray-400 py-12 text-[13px]">No posts yet</div>
              ) : (
            ...
            >
              ...
              <div className="bg-gray-50 rounded-2xl p-4 mb-4 border border-gray-100">
                ...
                <div className="flex items-center justify-between mb-2 text-[13px]">
                  ...
                  <span className="font-semibold">${creator.messagePrice}.00 USDC</span>
                </div>
                <div className="flex items-center justify-between text-[13px]">
                  <span className="text-gray-500">Network</span>
                  <span className="font-semibold">Base · ~$0.02 gas</span>
                </div>
              </div>
              ...
              <button
                onClick={() => setShowSubModal(false)}
                className="w-full py-3 text-gray-500 text-[13px] font-semibold mt-1"
              >
                Cancel
              </button>
            </div>
  ...
  return (
    <button
      ...
      className={`flex-1 py-3 text-[13px] font-semibold transition-all ${
        active ? "text-black border-b-2 border-black -mb-px" : "text-gray-400 hover:text-black"
      }`}
    >
      {label}
    </button>
...
function InlinePost({ post, creator }) {
  return (
    <article className="border-b border-gray-100 px-5 py-4">
      <div className="flex items-center gap-2 mb-3">
        <img src={creator.avatar} alt="" className="w-9 h-9 rounded-full object-cover" />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <span className="font-semibold text-[14px]">{creator.name}</span>
            {creator.verified && <VerifiedBadge size={12} />}
          </div>
          <span className="text-[11px] text-gray-500">{post.createdAt}</span>
        </div>
      </div>
      <p className="text-[14px] whitespace-pre-line leading-relaxed mb-3">{post.content}</p>
      {post.mediaUrl && (
        <div className="rounded-2xl overflow-hidden border border-gray-100">
          <img src={post.mediaUrl} alt="" className="w-full aspect-[4/3] object-cover" />
        </div>
      )}
      <div className="flex items-center gap-6 text-gray-500 mt-3 text-[12px] font-semibold">
...
function InfoRow({ label, value, mono }) {
  return (
    <div className="flex items-start justify-between gap-4 py-2 border-b border-gray-100 last:border-0">
      <span className="text-[13px] text-gray-500 font-medium shrink-0">{label}</span>
      <span className={`text-[13px] text-black font-semibold text-right ${mono ? "font-mono" : ""}`}>
        {value}
      </span>
    </div>
  );
