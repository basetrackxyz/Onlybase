  ...
  return (
    <MobileContainer>
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-xl border-b border-gray-100 px-4 pt-4 pb-3 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold tracking-tight" data-testid="profile-title">
          You
        </h1>
        <button
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-5 pt-6 pb-5 border-b border-gray-100">
          <div className="flex items-start justify-between mb-4">
            ...
            />
            <button
              className="text-[12px] font-semibold px-4 py-2 rounded-full border border-gray-200 hover:bg-gray-50"
          ...
          <div className="flex items-center gap-1.5 mb-1">
            <h2 className="font-display text-xl font-bold tracking-tight">{currentUser.name}</h2>
            {currentUser.verified && <SealCheck weight="fill" size={18} className="text-[#0052FF]" />}
          </div>
          <p className="text-[13px] text-gray-500 mb-3">@{currentUser.handle}</p>
          ...
          <Link
            to="/swap"
            className="flex items-center gap-3 bg-black text-white rounded-2xl p-3 active:scale-[0.98] transition-transform"
        ...
        {!currentUser.verified && (
          <div className="px-4 pt-5">
            <div className="relative overflow-hidden rounded-2xl border border-gray-200 p-5 bg-gradient-to-br from-white to-gray-50">
              <div className="absolute -right-6 -bottom-6 opacity-[0.08]">
                <SealCheck weight="fill" size={140} className="text-[#0052FF]" />
              </div>
              <div className="relative">
                <div className="text-xs uppercase tracking-[0.2em] font-bold text-[#0052FF] mb-2">
                  Monetize
                </div>
        ...
        {/* Policy note */}
        <div className="px-5 py-5 text-[11px] text-gray-400 leading-relaxed">
      ...
      {showVerifyModal && (
        ...
        >
          <div
            className="w-full bg-white rounded-t-3xl p-6 animate-in"
...
function MenuRow({ Icon, label, testid, danger }) {
  return (
    <button
      className="w-full flex items-center gap-3 py-3 px-3 -mx-3 rounded-xl active:bg-gray-50 transition-colors"
      data-testid={testid}
    >
      <div
        className={`w-9 h-9 rounded-full flex items-center justify-center ${
          danger ? "bg-red-50 text-red-500" : "bg-gray-100 text-black"
        }`}
      >
        <Icon size={16} weight="fill" />
      </div>
      <span className={`flex-1 text-left text-[14px] font-semibold ${danger ? "text-red-500" : ""}`}>
        {label}
      </span>
